...
$(document).ready(function () {
    ...
    function updateQuantity(newVal) {
        ...
        var totalPersTax = (parseFloat($("#totalPersTax").val()) * newVal * price) / 100;
        var totalAmountTax = parseFloat($("#totalAmountTax").val());
        tax_total = totalPersTax + totalAmountTax;
        var total = parseFloat(price) * newVal + parseFloat(tax_total);
    ...
    proQty.on("click", ".qtybtn", function () {
        ...
        if ($button.hasClass("inc")) {
            if (oldValue < tpo && oldValue < available) {
                ...
                newVal = oldValue;
                $('#quantityMsg').show().text('Max Limit Reached');
            }
        } else {
            if (oldValue > 1) {
                newVal = oldValue - 1;
    ...
    proQty.find("input").on("change", function () {
        ...
        if (newVal < 1) {
            newVal = 1;
        } else if (newVal > available || newVal > tpo) {
            newVal = tpo < available ? tpo : available;
...
function imagegallery(params) {
    ...
    document.getElementById('allSlide').style.display = 'block';
    $("#eventGalleryImg").attr("src", origin + "/images/upload/" + params);
}
...
$(document).ready(function () {
    ...
    function addMoneyWallet(token, amount, payment_type, currency = $("#walletCur").val()) {
        $.ajax({
            type: "post",
            url: "/user/deposite",
            headers: {
...
function plusSlides(n) {
    ...
    var allSlides = document.querySelectorAll('.imageGallery');

    for (var i = 0; i < allSlides.length; i++) {
        if (allSlides[i].src === document.getElementById("eventGalleryImg").src) {
            currentSlideIndex = i;
            break;
        }
    }
    var nextSlideIndex = currentSlideIndex + n;

    if (nextSlideIndex < 0) {
        nextSlideIndex = allSlides.length - 1;
    } else if (nextSlideIndex >= allSlides.length) {
        nextSlideIndex = 0;
    }
    document.getElementById("eventGalleryImg").src = allSlides[nextSlideIndex].src;
}
