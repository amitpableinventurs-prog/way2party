...
@section('content')
                            ...
                            <form method="post" class="event-form" action="{{ url('events') }}"
                                ...
                                <div class="form-group">
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        @if ($openai_switch == 1)
                                            <button type="button" id="generateDescBtn" class="btn btn-link position-relative">
                                                <i class="fas fa-magic px-2  wand-icon"></i>
                                                <i class="fas fa-circle-notch px-2 spinner-icon d-none"></i>
                                                {{ __('Auto Generate') }}
                                            </button>
                                        @endif
                                    </div>
    ...
    @if ($openai_switch == 1)
        <script>
            ...
            $('#generateDescBtn').on('click', function (e) {
                e.preventDefault();
                ...
                if (missing.length) {
                    alert('To auto-generate a description please fill: ' + missing.join(', '));
                    return;
                }

                const extraPoints = prompt(
                    'Great! Anything specific you’d like highlighted? (optional)',
                    ''
                );
                if (extraPoints === null) return;     // user pressed “Cancel”
                ...
                const payload = {
                    ...
                    end_time,
                    tags,
                    key_points: extraPoints.trim(),
                    people: $('#people').val(),
                    _token: '{{ csrf_token() }}'
                };

                $.ajax({
                    url:      "{{ route('events.generateDescription') }}",
                    method:   'POST',
                    data:     payload,
                    dataType: 'json',

                    beforeSend() {
                        btn.find('.wand-icon').addClass('d-none');
                        btn.find('.spinner-icon').removeClass('d-none');
                    },

                    complete() {
                        btn.find('.spinner-icon').addClass('d-none');
                        btn.find('.wand-icon').removeClass('d-none');
                    },
                    ...
                    success(res) {
                        ...
                        // 1) Summernote?
                        if ($.fn.summernote && $('.textarea_editor').next('.note-editor').length) {
                            $('.textarea_editor').summernote('code', text);
                            return;
                        }
                    },

                    error() {
                        alert('Could not generate a description right now.');
                    }
                });
            });
        </script>
