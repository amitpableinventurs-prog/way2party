                ...
                <div class="invoice-print">
                    ...
                    <div class="row ticket-header pb-3 mb-3">
                        <div class="col-lg-6 print-left">
                            ...
                            <p><span>{{ __('Booked For') }}: </span>
                                <b>
                                    @if ($ticket->order->ticket_date != '')
                                    {{ Carbon\Carbon::parse($ticket->order->ticket_date)->format('d M Y') }}
                                    @else
                                    {{ Carbon\Carbon::parse($ticket->order->event->start_time)->format('d M Y') . ' To '. Carbon\Carbon::parse($ticket->order->event->end_time)->format('d M Y') }}
                                    @endif
                                </b>
                            </p>
