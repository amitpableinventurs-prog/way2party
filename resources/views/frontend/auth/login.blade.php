                        ...
                        <label for="default-radio-1" class="w-full">
                            <div
                                ...
                                <input id="default-radio-1" type="radio" value="user" checked name="type"
                                    class="w-5 h-5 mr-2 border border-gray-light hover:border-gray-light focus:outline-none" onchange="updateGoogleLoginBtn(); updateDefaultCreds()">
                                <span>{{ __('User') }}</span>
                            </div>
                        </label>
                        <label for="default-radio-2" class="w-full">
                            <div
                                ...
                                <input id="default-radio-2" type="radio" value="org" name="type"
                                    class="w-5 h-5 mr-2 border select border-gray-light hover:border-gray-light focus:outline-none" onchange="updateGoogleLoginBtn(); updateDefaultCreds()">
...
<script>
    ...
    if ("{{ env('APP_DEBUG') }}" == true) {
        function updateDefaultCreds() {
            const $selectedType = document.querySelector('input[name="type"]:checked');
            if ($selectedType.value == 'user') {
                document.getElementById('email').value = "{{ env('DEFAULT_CUSTOMER_EMAIL') }}";
                document.getElementById('password').value = "{{ env('DEFAULT_CUSTOMER_PASSWORD') }}";
            } else if ($selectedType.value == 'org') {
                document.getElementById('email').value = "{{ env('DEFAULT_ORGANIZER_EMAIL') }}";
                document.getElementById('password').value = "{{ env('DEFAULT_ORGANIZER_PASSWORD') }}";
            } else {
                document.getElementById('email').value = "{{ env('DEFAULT_CUSTOMER_EMAIL') }}";
                document.getElementById('password').value = "{{ env('DEFAULT_CUSTOMER_PASSWORD') }}";
            }
