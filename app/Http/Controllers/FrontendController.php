...
class FrontendController extends Controller
{
            ...
            if (Auth::attempt($userdata, $remember)) {
                if (Auth::user()->hasRole('Organizer')) {
                    ...
                    if ($user->provider == 'GOOGLE') {
                        return redirect()->route('user.login')->with('error', 'You are registered with Google Sign-in.');
                    }
                    $setting = Setting::first();
    ...
    public function redirectToGoogleForSignin()
    {
        $type = request()->get('type');
        $googleSettings = Setting::first(['google_client_id', 'google_client_secret', 'enable_google_signin']);
        if ($googleSettings->enable_google_signin == 1) {
            $config = [
                'client_id' => $googleSettings->google_client_id,
                'client_secret' => $googleSettings->google_client_secret,
                'redirect' => url('login/google/callback'),
            ];
            return Socialite::buildProvider(GoogleProvider::class, $config)->with(['state' => $type])->redirect();
        } else {
            return redirect()->route('user.login')->with('error', 'Google sign in is not enabled.');
        }
    }
    ...
    public function loginWithGoogleCallback(Request $request)
    {
        ...
        $googleUser = Socialite::buildProvider(GoogleProvider::class, $config)->stateless()->user();
        ...
        switch ($type) {
            case 'organizer':
                ...
                if (!$user) {
                    $user = User::create(['provider_token' => $googleUser->getId(),
                        'first_name' => $googleUser->given_name,
                        'last_name' => $googleUser->family_name,
                        'organization_name' => $googleUser->getName(),
                        'email' => $googleUser->getEmail(),
                        'provider' => 'GOOGLE',
                        'provider_token' => $googleUser->getId(),
                ...
                // user already exists
                    ...
                    if ($user->status == 0) {
                        return redirect()->route('user.login')->with('error', 'Your account is blocked by admin.');
                    }
                    if (!$user->hasRole('Organizer')) {
                        return redirect()->route('user.login')->with('error', 'You are not registered as an organizer.');
                    }
                    $user->update(['provider_token' => $googleUser->getId(),
                    ]);
                };
            ...
            case 'user':
                $user = AppUser::where('email', $googleUser->getEmail())->first();
                if (!$user) {
                    $user = AppUser::create(['name' => $googleUser['given_name'],
                        'last_name' => $googleUser['family_name'],
                        'phone' => '',
                        'email' => $googleUser->getEmail(),
                        'password' => Hash::make($googleUser->getEmail()),
                ...
                Auth::guard('appuser')->loginUsingId($user->id);
                break;

            default:
                return redirect()->route('user.login')->with('error', 'You are not authorized to login');
        }
    ...
    public function sentMessageToAdmin(Request $request)
    {
        ...
        if ($setting->mail_notification == 1) {
            try {
                Mail::send('emails.message', ['data' => $data], function ($message) use ($data, $setting) {
    ...
    public function flutterwaveWebhook(Request $request){
        $reqData = $request->all();
        $secretHash = PaymentSetting::first()->raveWebhookSecretHash;
        $signature = $request->header('verif-hash');
        if (!$signature || ($signature !== $secretHash)) {
            return false;
        }
        if ("eventrightpro" != $reqData['meta_data']['platform']) {
            return false;
        }
        if ($reqData['event'] == "charge.completed" && $reqData['data']['status'] == "successful"){

            if ($reqData['meta_data']['payment_for'] == 'ticket'){
                $ticketId = $reqData['meta_data']['ticket_id'];
                $payment = $reqData['meta_data']['payment'];
                $tax = $reqData['meta_data']['tax'];
                $couponCode = $reqData['meta_data']['coupon_code'] ?? null;
                $selectedSeatsId = $reqData['meta_data']['selectedSeatsId'] ?? null;
                $selectedSeats = $reqData['meta_data']['selectedSeats'] ?? null;
                $quantity = $reqData['meta_data']['quantity'];
                $customer = $reqData['data']['customer'];
                $paymentType = $reqData['meta_data']['payment_type'];

                $ticket = Ticket::findOrFail($ticketId);
                $event = Event::find($ticket->event_id);

                $org = User::find($event->user_id);
                $user = AppUser::where('email', $customer['email'])->first();
                $data['order_id'] = '#' . rand(9999, 100000);
                $data['event_id'] = $event->id;
                ...
                $data['book_seats'] = isset($selectedSeatsId) ? $selectedSeatsId : null;
                $data['seat_details'] = isset($selectedSeats) ? $selectedSeats : null;
                $order = Order::create($data);
                $module = Module::where('module', 'Seatmap')->first();
                if ($module->is_enable == 1 && $module->is_install == 1) {
                    $seats = explode(',', $data['selectedSeatsId']);
            ...
            if ($reqData['meta_data']['payment_for'] == 'wallet') {
                $user = AppUser::where('email', $reqData['data']['customer']['email'])->first();
                $user->deposit(
                    $reqData['data']['amount'],[
                    'payment_mode' => 'FLUTTERWAVE',
                    'currency' => $reqData['data']['currency'],
                    'token' => $reqData['data']['id']
                ]);
            }
        }
        return true;
