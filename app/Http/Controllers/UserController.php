...
class UserController extends Controller
{
    ...
    public function generateLoginTokenForAppUser($id)
    {
    ...
    public function generateLoginTokenForUser($id)
    {
        $user = User::find($id);
        $token = $user->createToken('eventRight')->accessToken;
        return response()->json(['token' => $token, 'success' => true]);
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            "roles"    => "bail|required|array|min:1|max:1",
            'roles.*' => 'bail|required|string|distinct|min:1|max:1',
        ]);
        $data = $request->all();
        ...
        $role = Role::where('id', $request->roles[0])->first()->name;
    ...
    public function changeLanguage($lang)
    {
        ...
        session()->put('code', $language->code ?? 'en');
