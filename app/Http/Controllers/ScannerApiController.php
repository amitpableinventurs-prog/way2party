...
class ScannerApiController extends Controller
{
    ...
    public function events()
    {
        ...
        $data = $data->filter(function ($item) {
            return Auth::user()->id == $item->user_id || str_contains($item->scanner_id, Auth::user()->id);
        })->values();
    ...
    public function scanTicket($code, $event_id)
    {
        ...
        if ($ticket->allday == 0) {
            if (Carbon::parse($order->ticket_date)->format("Y-m-d") != Carbon::now()->format("Y-m-d")) {
                return response()->json(['msg' => 'Ticket date is not match.', 'data' => $data, 'success' => false], 200);
            }
        }else{
            if (!Carbon::now()->between(Carbon::parse($event->start_time), Carbon::parse($event->end_time))) {
                return response()->json(['msg' => 'Ticket date is not match.', 'data' => $data, 'success' => false], 200);
            }
        }
        if ($child->paid == 0) {
