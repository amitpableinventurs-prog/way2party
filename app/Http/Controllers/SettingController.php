...
class SettingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('favicon')) {
            $request->validate([
                'favicon' => 'required|image|mimes:jpeg,png,jpg,gif,svg|max:3048',
            ]);
    ...
    public function saveMailSetting(Request $request)
    {
        ...
        $this->updateEnvFile([
            ...
            'MAIL_FROM_ADDRESS' => $request->input('mail_sender_email') ?? $request->input('sender_email'),
        ]);
    ...
    public function saveGoogleSigninSetting(Request $request)
    {
        $data = $request->validate([
            'enable_google_signin' => 'nullable',
            'google_client_id' => 'bail|required_if:enable_google_signin,1',
            'google_client_secret' => 'bail|required_if:enable_google_signin,1',
        ], [
            'google_client_id.required_if' => __('The Client ID field is required when Google Sign-in is enabled.'),
            'google_client_secret.required_if' => __('The Client Secret field is required when Google Sign-in is enabled.'),
        ]);

        if (!isset($request->enable_google_signin)) {
            $data['enable_google_signin'] = 0;
        }
        Setting::find(1)->update($data);
        return redirect('admin-setting')->withStatus(__('Setting saved successfully.'));
    }
    ...
    {
        $data = $request->all();
        if (!isset($request->show_link_banner)) {
            $data['show_link_banner'] = 0;
        }
        if (!isset($request->show_event_report_form)) {
            $data['show_event_report_form'] = 0;
        }
        if (!isset($request->show_brands_carousel)) {
            $data['show_brands_carousel'] = 0;
        }
    ...
    public function savePaymentSetting(Request $request)
    {
        $request->validate([
            ...
            'raveWebhookSecretHash' => 'bail|required_if:flutterwave,1',
        ]);
        $data = $request->all();
    ...
    public function maintenanceMode(Request $request)
    {
        ...
        if ($request->hasFile('maintenance_bgimg')) {
            $request->validate([
                'maintenance_bgimg' => 'required|mimes:jpeg,png,jpg,gif,svg|max:3048',
            ]);
    ...
    public function saveDebug(Request $request)
    {
        if ($request->app_debug == 1) {
            $this->updateEnvFile(['APP_DEBUG' => 'true']);
        } else {
    ...
    public function openai_settings(Request $request)
    {
        abort_if(Gate::denies('settings_access'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        $request->validate([
            'openai_key' => 'required_if:openai_switch,1',
        ], [
            'openai_key.required_if' => 'OpenAI API Key is required',
        ]);
        Setting::first()->update([
            'openai_key' => $request->openai_key,
            'openai_switch' => $request->has('openai_switch') ? 1 : 0,
            'openai_selected_model' => $request->openai_selected_model,
        ]);

        return redirect()->back()->withStatus(__('OpenAI Settings Updated Successfully.'));
    }

    public function openai_test(Request $request)
    {
        abort_if(Gate::denies('settings_access'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        $request->validate([
            'openai_key' => 'required_if:openai_switch,1',
            'max_tokens' => 'required_if:openai_switch,1',
        ], [
            'openai_key.required_if' => 'OpenAI API Key is required',
            'max_tokens.required_if' => 'Max Token amount is required',
        ]);
        try {
            $apiKey = $request->openai_key;
            $selected_model = $request->openai_selected_model;
            ...
            if (isset($response->error)) {
                return response()->json(['msg' => $response->error->message, 'success' => false], 500);
            }
            return response()->json(['msg' => 'Configuration tested ok', 'success' => true], 200);
        } catch (\Throwable $th) {
            return response()->json(['msg' => 'Something went wrong', 'success' => false, 'error' => $th->getMessage()], 500);
        }
    }
}
