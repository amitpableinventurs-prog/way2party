...
class ApiController extends Controller
{
    ...
    public function eventTickets($id)
    {
        ...
        $data['organization'] = User::findOrFail($event->user_id)->organization_name;
        $timezone = Setting::findOrFail(1)->timezone;
        $date = Carbon::now($timezone);
    ...
    public function userLikes()
    {
        ...
        $data = Event::whereIn('id', array_filter(explode(',', Auth::user()->favorite)))->where([['status', 1], ['is_deleted', 0],['end_time', '>', Carbon::now()]])->orderBy('id', 'DESC')->get();
        foreach ($data as $value) {
            $value->description =  str_replace("&nbsp;", " ", strip_tags($value->description));
            $value->time = $value->start_time->format('d F Y h:i a');
        }
    ...
    {
        $general = Setting::find(1, ['app_name', 'app_version', 'logo', 'currency', 'onesignal_app_id', 'onesignal_project_number', 'help_center', 'appuser_privacy_policy', 'cookie_policy', 'terms_services', 'acknowledgement', 'currency_sybmol', 'user_verify']);
    ...
    {
        $data['upcoming'] = Order::with(['event', 'ticket'])
            ->where('customer_id', Auth::user()->id)
            ->where(function ($query) {
            $query->where('ticket_date', '>=', now())
            ->orWhere(function ($subQuery) {
                $subQuery->whereNull('ticket_date')
                    ->whereHas('event', function ($eventQuery) {
                        $eventQuery->where('end_time', '>=', now());
                    });
            });
            })
            ->orderBy('id', 'DESC')->get();
